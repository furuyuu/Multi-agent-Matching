...
3エージェント実験は [notebooks/experiment.ipynb](notebooks/experiment.ipynb) から実行します。4エージェント実験は [notebooks/experiment_4agents.ipynb](notebooks/experiment_4agents.ipynb) から実行します。結果は repository root、つまりこの `research` フォルダ内の `results/YYYYMMDD_HHMMSS_JST_seed_X/` に保存されます。CSV は解析用に保存し、同時に `summary.html` も生成して、画像と表をブラウザで確認できるようにしています。

4エージェント実験では、各3-agent triple の k-wise RRWM 結果を単独評価するだけでなく、reference agent への対応に集約してから全 pair を合成する `Consistent 3-wise RRWM` も出力します。これは k-wise MGM 論文の「各 k-wise indicator tensor を reference graph への対応へ分解し、`W = U^T U` で全体対応を構成する」考え方に寄せた one-shot 統合です。
...
1. 3エージェント実験は `notebooks/experiment.ipynb`、4エージェント実験は `notebooks/experiment_4agents.ipynb` を開く。
2. 上から順にセルを実行する。
